...
# 2. Check Correlation Matrix (برای متغیرهای عددی)
# ابتدا لیست ستون‌های عددی را جدا می‌کنیم
numeric_features = df.select_dtypes(include=[np.number]).columns.tolist()
# ستون Target را از لیست ویژگی‌ها حذف می‌کنیم
...
# 3. Distribution of Numeric Features (هیستوگرم)
...
# (با توجه به Data Dictionary، حتی اگر اعداد هستند، برخی categorical اند)
...
# برای داده‌های عددی: Missing Values را با میانه پر می‌کنیم و سپس Scale می‌کنیم.
# برای داده‌های категоوری: Missing Values را با مکررترین مقدار (mode) پر می‌کنیم. چون مقدار آن‌ها عددی است، نیازی به One-Hot Encoding نیست.
numeric_transformer = Pipeline(steps=[
    ('imputer', SimpleImputer(strategy='median')), # یا KNNImputer(n_neighbors=5)
    ('scaler', StandardScaler()) # یا MinMaxScaler
])

categorical_transformer = Pipeline(steps=[
    ('imputer', SimpleImputer(strategy='most_frequent')),
    # از آنجایی که مقادیر از قبل عددی هستند، encoder نیاز نیست.
])
...
# (Optional) اگر خواستیم دیتای processed را در قالب DataFrame ببینیم
# چون ColumnTransformer اسم ستون‌ها را از بین می‌برد، مجدداً می‌سازیم
...
# 3. ROC Curve and AUC (اگر مدل probability داشته باشد)
fpr, tpr, thresholds = roc_curve(y_test, y_pred_proba, pos_label=2) # pos_label=2 یعنی کلاس "زنده" کلاس مثبت در نظر گرفته شود.
...
# این ترتیب ستون‌ها باید دقیقاً مشابه داده اصلی باشه: 'Age', 'Sex', 'Steroid', ...
...
confidence = prediction_proba[0].max() * 100 # اعتماد به نفس مدل
